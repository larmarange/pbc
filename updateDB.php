<?php
	...
	if($thisMD5==$prevMD5) {
		...
		setupTable('depenses', "create table if not exists `depenses` (   `id` INT unsigned not null auto_increment , primary key (`id`), `convention` INT unsigned not null , `ligne_budgetaire` INT unsigned null , `ligne_credit` INT unsigned not null , `date` DATE not null , `intitule` VARCHAR(100) not null , `reference` VARCHAR(40) null , `contrat` INT unsigned null , `beneficiaire` INT unsigned null , `montant` DECIMAL(10,2) not null , `statut` VARCHAR(40) not null default '1' , `ventilation` INT unsigned null , `notes` TEXT null , `verifie` VARCHAR(40) null ) CHARSET utf8", $silent);
