...
<?php }else{ ?>
	...
	<button class="btn btn-lg btn-default pull-right search_mode" type="submit" name="Filter_x" value="1">Switch to advanced search mode</button>
			...
			<div class="page-header"><h1>
				<a href="versements_view.php" style="text-decoration: none; color: inherit;">
					<img src="resources/table_icons/009-cash.png"> 					Versements (bailleur) Filters</a>
			</h1></div>
		...
		$j(function(){
			...
			$j("#filter_3").select2({
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ return { s: term, p:page, t:"versements", f:"ligne_budgetaire" }; },
					results: function (resp, page) { return resp; }
				}
			}).on('change', function(e){
	...
	<div class="row vspacer-lg" style="border-bottom: dotted 2px #DDD;" >
		...
		<div class="col-xs-9 col-md-3 col-lg-2 vspacer-md">
			<input type="text"  class="form-control" id="from-date_4"  name="FilterValue[3]" value="<?php echo htmlspecialchars($FilterValue[3]); ?>" size="10">
		</div>
		...
		<div class="col-xs-9 col-md-3 col-lg-2 vspacer-md">
			<input type="text" class="form-control" id="to-date_4" name="FilterValue[4]" value="<?php echo htmlspecialchars($FilterValue[4]); ?>" size="10">
		</div>
	...
	<script>
		$j(function(){
			...
			$j("#from-date_4" ).on('dp.change' , function(e){
				...
				$j("#to-date_4 ").val(date.format('DD/MM/YYYY')).data("DateTimePicker").minDate(e.date);
			});
		});
	</script>
