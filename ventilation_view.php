...
// Download AppGini for free from https://bigprof.com/appgini/download/
	...
	if(strpos($x->HTML, '<!-- tv data below -->')){
		...
		$sumQuery = "select CONCAT('<span style=''color: ', IF(sum(`ventilation`.`accorde`) < 0, 'red', 'black'), ';''>', FORMAT(sum(`ventilation`.`accorde`), 2, 'ru_RU'), '</span>'), CONCAT('<span style=''color: ', IF(sum(`ventilation`.`reserve`) < 0, 'red', 'black'), ';''>', FORMAT(sum(`ventilation`.`reserve`), 2, 'ru_RU'), '</span>'), CONCAT('<span style=''color: ', IF(sum(`ventilation`.`liquide`) < 0, 'red', 'black'), ';''>', FORMAT(sum(`ventilation`.`liquide`), 2, 'ru_RU'), '</span>'), CONCAT('<span style=''color: ', IF(sum(`ventilation`.`utilise`) < 0, 'red', 'black'), ';''>', FORMAT(sum(`ventilation`.`utilise`), 2, 'ru_RU'), '</span>'), CONCAT('<span style=''color: ', IF(sum(`ventilation`.`reste_engager`) < 0, 'red', 'black'), ';''>', FORMAT(sum(`ventilation`.`reste_engager`), 2, 'ru_RU'), '</span>') from {$x->QueryFrom} {$QueryWhere}";
		$res = sql($sumQuery, $eo);
