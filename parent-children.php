		...
		$pcConfig = array(
			...
			'versements' => array(   
				'convention' => array(   
					...
					'query' => "SELECT `versements`.`id` as 'id', IF(    CHAR_LENGTH(`conventions1`.`nom`), CONCAT_WS('',   `conventions1`.`nom`), '') as 'convention', IF(    CHAR_LENGTH(`types_ligne1`.`gestionnaire`) || CHAR_LENGTH(`types_ligne1`.`type`), CONCAT_WS('',   `types_ligne1`.`gestionnaire`, ' - ', `types_ligne1`.`type`), '') as 'ligne_budgetaire', if(`versements`.`date`,date_format(`versements`.`date`,'%d/%m/%Y'),'') as 'date', `versements`.`intitule` as 'intitule', CONCAT('<span style=''color: ', IF(`versements`.`montant` < 0, 'red', 'black'), ';''>', FORMAT(`versements`.`montant`, 2, 'ru_RU'), '</span>') as 'montant', `versements`.`notes` as 'notes' FROM `versements` LEFT JOIN `conventions` as conventions1 ON `conventions1`.`id`=`versements`.`convention` LEFT JOIN `budgets` as budgets1 ON `budgets1`.`id`=`versements`.`ligne_budgetaire` LEFT JOIN `types_ligne` as types_ligne1 ON `types_ligne1`.`id`=`budgets1`.`type` "
				),
			...
			'fichiers' => array(   
				'convention' => array(   
					...
					'query' => "SELECT `fichiers`.`id` as 'id', IF(    CHAR_LENGTH(`conventions1`.`nom`), CONCAT_WS('',   `conventions1`.`nom`), '') as 'convention', `fichiers`.`titre` as 'titre', `fichiers`.`fichier` as 'fichier', `fichiers`.`notes` as 'notes' FROM `fichiers` LEFT JOIN `conventions` as conventions1 ON `conventions1`.`id`=`fichiers`.`convention` "
				)
			),
			'personnes' => array(   
			),
