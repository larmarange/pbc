...
function lignes_credits_insert(&$error_message = '') {
	global $Translation;
	...
	if(function_exists('lignes_credits_before_insert')) {
		$args = [];
		if(!lignes_credits_before_insert($data, getMemberInfo(), $args)) {
	...
	insert('lignes_credits', backtick_keys_once($data), $error);
	if($error)
		die("{$error}<br><a href=\"#\" onclick=\"history.go(-1);\">{$Translation['< back']}</a>");

	$recID = db_insert_id(db_link());

	update_calc_fields('lignes_credits', $recID, calculated_fields()['lignes_credits']);
	...
	if(!empty($_REQUEST['SelectedID'])) lignes_credits_copy_children($recID, $_REQUEST['SelectedID']);

	return $recID;
}

function lignes_credits_copy_children($destination_id, $source_id) {
	global $Translation;
...
function lignes_credits_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	...
	if(!check_record_permission('lignes_credits', $selected_id, 'delete')) {
	...
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `credits` WHERE `ligne_credit`='" . makeSafe($id[0]) . "'", $eo);
	...
	$res = sql("SELECT `id` FROM `lignes_credits` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `depenses` WHERE `ligne_credit`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
...
function lignes_credits_update(&$selected_id, &$error_message = '') {
	global $Translation;

	// mm: can member edit record?
	if(!check_record_permission('lignes_credits', $selected_id, 'edit')) return false;
	...
	// get existing values
	$old_data = getRecord('lignes_credits', $selected_id);
	...
	if(function_exists('lignes_credits_before_update')) {
		...
		if(!lignes_credits_before_update($data, getMemberInfo(), $args)) {
			if(isset($args['error_message'])) $error_message = $args['error_message'];
			return false;
		}
	}
	...
	update_calc_fields('lignes_credits', $data['selectedID'], calculated_fields()['lignes_credits']);
