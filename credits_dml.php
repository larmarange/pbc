...
function credits_insert(&$error_message = '') {
	global $Translation;
	...
	$data = [
		...
		'ligne_credit' => Request::val('ligne_credit', ''),
	...
	if(function_exists('credits_before_insert')) {
		$args = [];
		if(!credits_before_insert($data, getMemberInfo(), $args)) {
	...
	insert('credits', backtick_keys_once($data), $error);
	if($error)
		die("{$error}<br><a href=\"#\" onclick=\"history.go(-1);\">{$Translation['< back']}</a>");

	$recID = db_insert_id(db_link());

	update_calc_fields('credits', $recID, calculated_fields()['credits']);
	...
	if(!empty($_REQUEST['SelectedID'])) credits_copy_children($recID, $_REQUEST['SelectedID']);

	return $recID;
}

function credits_copy_children($destination_id, $source_id) {
	global $Translation;
...
function credits_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	...
	if(!check_record_permission('credits', $selected_id, 'delete')) {
...
function credits_update(&$selected_id, &$error_message = '') {
	global $Translation;

	// mm: can member edit record?
	if(!check_record_permission('credits', $selected_id, 'edit')) return false;
	...
	// get existing values
	$old_data = getRecord('credits', $selected_id);
	...
	if(function_exists('credits_before_update')) {
		...
		if(!credits_before_update($data, getMemberInfo(), $args)) {
			if(isset($args['error_message'])) $error_message = $args['error_message'];
			return false;
		}
	}
	...
	update_calc_fields('credits', $data['selectedID'], calculated_fields()['credits']);
...
function credits_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = '') {
	...
	$lookup_fields = array('convention' => array('conventions', 'Convention'), 'ligne_budgetaire' => array('budgets', 'Ligne Budg&#233;taire'), 'ligne_credit' => array('lignes_credits', 'Ligne de cr&#233;dit (CFI) - Exercice'), );
