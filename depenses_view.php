...
// Download AppGini for free from https://bigprof.com/appgini/download/
	...
	@include_once("{$currDir}/hooks/depenses.php");
	include_once("{$currDir}/depenses_dml.php");
	...
	$x->filterers = ['convention' => 'Convention', 'ligne_budgetaire' => 'Ligne Budg&#233;taire', 'ligne_credit' => 'Ligne de cr&#233;dit (CFI) - Exercice', 'contrat' => 'Contrat (si salaire)', 'beneficiaire' => 'B&#233;n&#233;ficiaire', 'ventilation' => 'Ventilation Budg&#233;taire', ];
	...
	$x->ColCaption = ['Convention', 'Ligne Budg&#233;taire', 'Ligne de cr&#233;dit (CFI) - Exercice', 'Date', 'Intitul&#233;', 'R&#233;ference', 'Contrat (si salaire)', 'B&#233;n&#233;ficiaire', 'Montant', 'Liquid&#233;e ?', 'Ventilation Budg&#233;taire', 'Notes', 'V&#233;rifi&#233;e ?', ];
	$x->ColFieldName = ['convention', 'ligne_budgetaire', 'ligne_credit', 'date', 'intitule', 'reference', 'contrat', 'beneficiaire', 'montant', 'liquidee', 'ventilation', 'notes', 'verifie', ];
	...
	if(strpos($x->HTML, '<!-- tv data below -->')) {
		...
		if($row = db_fetch_row($res)) {
			...
			$sumRow .= '<td class="depenses-convention sum"></td>';
			$sumRow .= '<td class="depenses-ligne_budgetaire sum"></td>';
			$sumRow .= '<td class="depenses-ligne_credit sum"></td>';
			$sumRow .= '<td class="depenses-date sum"></td>';
			$sumRow .= '<td class="depenses-intitule sum"></td>';
			$sumRow .= '<td class="depenses-reference sum"></td>';
			$sumRow .= '<td class="depenses-contrat sum"></td>';
			$sumRow .= '<td class="depenses-beneficiaire sum"></td>';
			$sumRow .= "<td class=\"depenses-montant text-right sum\">{$row[0]}</td>";
			$sumRow .= '<td class="depenses-liquidee sum"></td>';
			$sumRow .= '<td class="depenses-ventilation sum"></td>';
			$sumRow .= '<td class="depenses-notes sum"></td>';
			$sumRow .= '<td class="depenses-verifie sum"></td>';
			$sumRow .= '</tr>';
