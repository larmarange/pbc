...
function recrutements_insert(&$error_message = '') {
	global $Translation;
	...
	$data = [
		...
		'beneficiaire' => Request::val('beneficiaire', ''),
		'date_debut' => Request::dateComponents('date_debut', ''),
		'date_fin' => Request::dateComponents('date_fin', ''),
		'duree' => Request::val('duree', ''),
		'ligne_budgetaire' => Request::val('ligne_budgetaire', ''),
		'ventilation' => Request::val('ventilation', ''),
		'notes' => Request::val('notes', ''),
		'previsionnel' => Request::val('previsionnel', ''),
		'depense' => Request::val('depense', ''),
		'reservation_salaire' => Request::val('reservation_salaire', ''),
		'prop_dp' => Request::val('prop_dp', ''),
	];
	...
	if(function_exists('recrutements_before_insert')) {
		$args = [];
		if(!recrutements_before_insert($data, getMemberInfo(), $args)) {
	...
	update_calc_fields('recrutements', $recID, calculated_fields()['recrutements']);
...
function recrutements_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	...
	if(!check_record_permission('recrutements', $selected_id, 'delete')) {
	...
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `depenses` WHERE `contrat`='" . makeSafe($id[0]) . "'", $eo);
...
function recrutements_update(&$selected_id, &$error_message = '') {
	global $Translation;

	// mm: can member edit record?
	if(!check_record_permission('recrutements', $selected_id, 'edit')) return false;
	...
	// get existing values
	$old_data = getRecord('recrutements', $selected_id);
	...
	if(function_exists('recrutements_before_update')) {
		...
		if(!recrutements_before_update($data, getMemberInfo(), $args)) {
			if(isset($args['error_message'])) $error_message = $args['error_message'];
			return false;
		}
	}
	...
	update_calc_fields('recrutements', $data['selectedID'], calculated_fields()['recrutements']);
