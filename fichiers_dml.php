...
function fichiers_insert(){
	...
	if($_REQUEST['SelectedID']){
		...
		if($row = db_fetch_assoc($res)){
			if(!$data['fichier']) $data['fichier'] = makeSafe($row['fichier']);
		}
	}
	...
	sql('insert into `fichiers` set       `convention`=' . (($data['convention'] !== '' && $data['convention'] !== NULL) ? "'{$data['convention']}'" : 'NULL') . ', `titre`=' . (($data['titre'] !== '' && $data['titre'] !== NULL) ? "'{$data['titre']}'" : 'NULL') . ', ' . ($data['fichier'] != '' ? "`fichier`='{$data['fichier']}'" : '`fichier`=NULL') . ', `notes`=' . (($data['notes'] !== '' && $data['notes'] !== NULL) ? "'{$data['notes']}'" : 'NULL'), $o);
	if($o['error']!=''){
		echo $o['error'];
		echo "<a href=\"fichiers_view.php?addNew_x=1\">{$Translation['< back']}</a>";
		exit;
	}
...
function fichiers_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = ''){
	...
	$templateCode = str_replace('<%%UPLOADFILE(fichier)%%>', ($noUploads ? '' : '<input type=hidden name=MAX_FILE_SIZE value=1024000000>'.$Translation['upload image'].' <input type="file" name="fichier" id="fichier">'), $templateCode);
