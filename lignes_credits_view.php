...
// Download AppGini for free from https://bigprof.com/appgini/download/
	...
	$x->QueryFieldsTV = array(   
		...
		"CONCAT('$', FORMAT(`lignes_credits`.`reserve`, 2))" => "reserve",
	...
	if(strpos($x->HTML, '<!-- tv data below -->')){
		...
		$sumQuery = "select CONCAT('$', FORMAT(sum(`lignes_credits`.`ouvert`), 2)), CONCAT('$', FORMAT(sum(`lignes_credits`.`reserve`), 2)), CONCAT('$', FORMAT(sum(`lignes_credits`.`liquide`), 2)), CONCAT('$', FORMAT(sum(`lignes_credits`.`utilise`), 2)), CONCAT('$', FORMAT(sum(`lignes_credits`.`disponible`), 2)) from {$x->QueryFrom} {$QueryWhere}";
		$res = sql($sumQuery, $eo);
