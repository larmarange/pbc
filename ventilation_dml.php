...
function ventilation_insert() {
	...
	$data['rubrique'] = $_REQUEST['rubrique'];
		if($data['rubrique'] == empty_lookup_value) { $data['rubrique'] = ''; }
	...
	if(function_exists('ventilation_before_insert')) {
		$args = array();
		if(!ventilation_before_insert($data, getMemberInfo(), $args)) { return false; }
	}
...
function ventilation_delete($selected_id, $AllowDeleteOfParents=false, $skipChecks=false) {
	...
	$arrPerm=getTablePermissions('ventilation');
	$ownerGroupID=sqlValue("select groupID from membership_userrecords where tableName='ventilation' and pkValue='$selected_id'");
	$ownerMemberID=sqlValue("select lcase(memberID) from membership_userrecords where tableName='ventilation' and pkValue='$selected_id'");
	...
	$res = sql("select `id` from `ventilation` where `id`='$selected_id'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("select count(1) from `depenses` where `ventilation`='".addslashes($id[0])."'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
...
function ventilation_update($selected_id) {
	global $Translation;
	...
	$data['rubrique'] = makeSafe($_REQUEST['rubrique']);
		if($data['rubrique'] == empty_lookup_value) { $data['rubrique'] = ''; }
	...
	if(function_exists('ventilation_before_update')) {
		$args = array();
		if(!ventilation_before_update($data, getMemberInfo(), $args)) { return false; }
	}
...
function ventilation_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = '') {
	...
	$filterer_rubrique = thisOr(undo_magic_quotes($_REQUEST['filterer_rubrique']), '');
