...
// Data functions (insert, update, delete, form) for table ventilation

// This script and data application were generated by AppGini 5.92
// Download AppGini for free from https://bigprof.com/appgini/download/

function ventilation_insert(&$error_message = '') {
	global $Translation;

	// mm: can member insert record?
	$arrPerm = getTablePermissions('ventilation');
	if(!$arrPerm['insert']) return false;

	$data = [
		'convention' => Request::val('convention', ''),
		'rubrique' => Request::val('rubrique', ''),
	...
	// hook: ventilation_before_insert
	if(function_exists('ventilation_before_insert')) {
		$args = [];
		if(!ventilation_before_insert($data, getMemberInfo(), $args)) {
	...
	insert('ventilation', backtick_keys_once($data), $error);
	if($error)
		die("{$error}<br><a href=\"#\" onclick=\"history.go(-1);\">{$Translation['< back']}</a>");

	$recID = db_insert_id(db_link());

	update_calc_fields('ventilation', $recID, calculated_fields()['ventilation']);

	// hook: ventilation_after_insert
	if(function_exists('ventilation_after_insert')) {
		$res = sql("SELECT * FROM `ventilation` WHERE `id`='" . makeSafe($recID, false) . "' LIMIT 1", $eo);
		if($row = db_fetch_assoc($res)) {
			$data = array_map('makeSafe', $row);
		}
		$data['selectedID'] = makeSafe($recID, false);
		$args=[];
		if(!ventilation_after_insert($data, getMemberInfo(), $args)) { return $recID; }
	}

	// mm: save ownership data
	set_record_owner('ventilation', $recID, getLoggedMemberID());

	// if this record is a copy of another record, copy children if applicable
	if(!empty($_REQUEST['SelectedID'])) ventilation_copy_children($recID, $_REQUEST['SelectedID']);

	return $recID;
}

function ventilation_copy_children($destination_id, $source_id) {
	global $Translation;
...
function ventilation_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	// insure referential integrity ...
	global $Translation;
	$selected_id = makeSafe($selected_id);

	// mm: can member delete record?
	if(!check_record_permission('ventilation', $selected_id, 'delete')) {
		return $Translation['You don\'t have enough permissions to delete this record'];
	}

	// hook: ventilation_before_delete
	if(function_exists('ventilation_before_delete')) {
		$args = [];
	...
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `recrutements` WHERE `ventilation`='" . makeSafe($id[0]) . "'", $eo);
	...
	$res = sql("SELECT `id` FROM `ventilation` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `depenses` WHERE `ventilation`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	sql("DELETE FROM `ventilation` WHERE `id`='{$selected_id}'", $eo);

	// hook: ventilation_after_delete
	if(function_exists('ventilation_after_delete')) {
		$args = [];
		ventilation_after_delete($selected_id, getMemberInfo(), $args);
	}

	// mm: delete ownership data
	sql("DELETE FROM `membership_userrecords` WHERE `tableName`='ventilation' AND `pkValue`='{$selected_id}'", $eo);
}

function ventilation_update(&$selected_id, &$error_message = '') {
	global $Translation;

	// mm: can member edit record?
	if(!check_record_permission('ventilation', $selected_id, 'edit')) return false;
	...
	// get existing values
	$old_data = getRecord('ventilation', $selected_id);
	...
	// hook: ventilation_before_update
	if(function_exists('ventilation_before_update')) {
		$args = ['old_data' => $old_data];
		if(!ventilation_before_update($data, getMemberInfo(), $args)) {
			if(isset($args['error_message'])) $error_message = $args['error_message'];
			return false;
		}
	}
	...
	update_calc_fields('ventilation', $data['selectedID'], calculated_fields()['ventilation']);

	// hook: ventilation_after_update
	if(function_exists('ventilation_after_update')) {
		$res = sql("SELECT * FROM `ventilation` WHERE `id`='{$data['selectedID']}' LIMIT 1", $eo);
		if($row = db_fetch_assoc($res)) $data = array_map('makeSafe', $row);

		$data['selectedID'] = $data['id'];
		$args = ['old_data' => $old_data];
		if(!ventilation_after_update($data, getMemberInfo(), $args)) return;
	}

	// mm: update ownership data
	sql("UPDATE `membership_userrecords` SET `dateUpdated`='" . time() . "' WHERE `tableName`='ventilation' AND `pkValue`='" . makeSafe($selected_id) . "'", $eo);
}

function ventilation_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = '') {
	...
	$filterer_rubrique = thisOr($_REQUEST['filterer_rubrique'], '');

	// populate filterers, starting from children to grand-parents
	if($filterer_rubrique && !$filterer_convention) $filterer_convention = sqlValue("select convention from rubriques where id='" . makeSafe($filterer_rubrique) . "'");

	// unique random identifier
	$rnd1 = ($dvprint ? rand(1000000, 9999999) : '');
	// combobox: convention
	$combo_convention = new DataCombo;
	// combobox: rubrique, filterable by: convention
	$combo_rubrique = new DataCombo;
	...
	if($selected_id) {
		...
		// mm: who is the owner?
		$ownerGroupID = sqlValue("SELECT `groupID` FROM `membership_userrecords` WHERE `tableName`='ventilation' AND `pkValue`='" . makeSafe($selected_id) . "'");
		$ownerMemberID = sqlValue("SELECT LCASE(`memberID`) FROM `membership_userrecords` WHERE `tableName`='ventilation' AND `pkValue`='" . makeSafe($selected_id) . "'");
		...
		$res = sql("SELECT * FROM `ventilation` WHERE `id`='" . makeSafe($selected_id) . "'", $eo);
		if(!($row = db_fetch_array($res))) {
			return error_message($Translation['No records found'], 'ventilation_view.php', false);
		}
		$combo_convention->SelectedData = $row['convention'];
		$combo_rubrique->SelectedData = $row['rubrique'];
		$urow = $row; /* unsanitized data */
		$hc = new CI_Input();
		$row = $hc->xss_clean($row); /* sanitize data */
	} else {
		$combo_convention->SelectedData = $filterer_convention;
		$combo_rubrique->SelectedData = $filterer_rubrique;
	}
	...
	$combo_rubrique->HTML = '<span id="rubrique-container' . $rnd1 . '"></span><input type="hidden" name="rubrique" id="rubrique' . $rnd1 . '" value="' . html_attr($combo_rubrique->SelectedData) . '">';
	$combo_rubrique->MatchText = '<span id="rubrique-container-readonly' . $rnd1 . '"></span><input type="hidden" name="rubrique" id="rubrique' . $rnd1 . '" value="' . html_attr($combo_rubrique->SelectedData) . '">';
	...
	<script>
		...
		AppGini.current_rubrique__RAND__ = { text: "", value: "<?php echo addslashes($selected_id ? $urow['rubrique'] : $filterer_rubrique); ?>"};

		jQuery(function() {
			setTimeout(function() {
				if(typeof(convention_reload__RAND__) == 'function') convention_reload__RAND__();
				<?php echo (!$AllowUpdate || $dvprint ? 'if(typeof(rubrique_reload__RAND__) == \'function\') rubrique_reload__RAND__(AppGini.current_convention__RAND__.value);' : ''); ?>
			...
			$j("#convention-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_convention__RAND__.value, t: 'ventilation', f: 'convention' },
						...
						if(typeof(rubrique_reload__RAND__) == 'function') rubrique_reload__RAND__(AppGini.current_convention__RAND__.value);
		...
		function rubrique_reload__RAND__(filterer_convention) {
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>

			$j("#rubrique-container__RAND__").select2({
				/* initial default value */
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_convention: filterer_convention, id: AppGini.current_rubrique__RAND__.value, t: 'ventilation', f: 'rubrique' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
							});
							$j('[name="rubrique"]').val(resp.results[0].id);
							$j('[id=rubrique-container-readonly__RAND__]').html('<span id="rubrique-match-text">' + resp.results[0].text + '</span>');
							if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=rubriques_view_parent]').hide(); } else { $j('.btn[id=rubriques_view_parent]').show(); }


							if(typeof(rubrique_update_autofills__RAND__) == 'function') rubrique_update_autofills__RAND__();
						}
					});
				},
				width: '100%',
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { /* */ return { filterer_convention: filterer_convention, s: term, p: page, t: 'ventilation', f: 'rubrique' }; },
					results: function(resp, page) { /* */ return resp; }
				},
				escapeMarkup: function(str) { /* */ return str; }
			}).on('change', function(e) {
				AppGini.current_rubrique__RAND__.value = e.added.id;
				AppGini.current_rubrique__RAND__.text = e.added.text;
				$j('[name="rubrique"]').val(e.added.id);
				if(e.added.id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=rubriques_view_parent]').hide(); } else { $j('.btn[id=rubriques_view_parent]').show(); }


				if(typeof(rubrique_update_autofills__RAND__) == 'function') rubrique_update_autofills__RAND__();
			});

			if(!$j("#rubrique-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_rubrique__RAND__.value, t: 'ventilation', f: 'rubrique' },
					success: function(resp) {
						$j('[name="rubrique"]').val(resp.results[0].id);
						$j('[id=rubrique-container-readonly__RAND__]').html('<span id="rubrique-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=rubriques_view_parent]').hide(); } else { $j('.btn[id=rubriques_view_parent]').show(); }
		...
		<?php } else { ?>
			...
			$j.ajax({
				...
				data: { id: AppGini.current_rubrique__RAND__.value, t: 'ventilation', f: 'rubrique' },
				success: function(resp) {
					$j('[id=rubrique-container__RAND__], [id=rubrique-container-readonly__RAND__]').html('<span id="rubrique-match-text">' + resp.results[0].text + '</span>');
					if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=rubriques_view_parent]').hide(); } else { $j('.btn[id=rubriques_view_parent]').show(); }
	...
	if($dvprint) {
		$template_file = is_file("./{$TemplateDVP}") ? "./{$TemplateDVP}" : './templates/ventilation_templateDVP.html';
		$templateCode = @file_get_contents($template_file);
	} else {
		$template_file = is_file("./{$TemplateDV}") ? "./{$TemplateDV}" : './templates/ventilation_templateDV.html';
		$templateCode = @file_get_contents($template_file);
	}
	...
	if($arrPerm[1] && !$selected_id) { // allow insert and no record selected?
		if(!$selected_id) $templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-success" id="insert" name="insert_x" value="1" onclick="return ventilation_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save New'] . '</button>', $templateCode);
		$templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-default" id="insert" name="insert_x" value="1" onclick="return ventilation_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save As Copy'] . '</button>', $templateCode);
	} else {
	...
	if($selected_id) {
		...
		if($AllowUpdate) {
			$templateCode = str_replace('<%%UPDATE_BUTTON%%>', '<button type="submit" class="btn btn-success btn-lg" id="update" name="update_x" value="1" onclick="return ventilation_validateData();" title="' . html_attr($Translation['Save Changes']) . '"><i class="glyphicon glyphicon-ok"></i> ' . $Translation['Save Changes'] . '</button>', $templateCode);
		} else {
	...
	if(($selected_id && !$AllowUpdate) || (!$selected_id && !$AllowInsert)) {
		...
		$jsReadOnly .= "\tjQuery('#rubrique').prop('disabled', true).css({ color: '#555', backgroundColor: '#fff' });\n";
	...
	$templateCode = str_replace('<%%COMBO(rubrique)%%>', $combo_rubrique->HTML, $templateCode);
	$templateCode = str_replace('<%%COMBOTEXT(rubrique)%%>', $combo_rubrique->MatchText, $templateCode);
	$templateCode = str_replace('<%%URLCOMBOTEXT(rubrique)%%>', urlencode($combo_rubrique->MatchText), $templateCode);

	/* lookup fields array: 'lookup field name' => array('parent table name', 'lookup field caption') */
	$lookup_fields = array('convention' => array('conventions', 'Convention'), 'rubrique' => array('rubriques', 'Rubrique de Ventilation'), );
	...
	$templateCode = str_replace('<%%UPLOADFILE(rubrique)%%>', '', $templateCode);
	...
	if($selected_id) {
		...
		if( $dvprint) $templateCode = str_replace('<%%VALUE(rubrique)%%>', safe_html($urow['rubrique']), $templateCode);
		if(!$dvprint) $templateCode = str_replace('<%%VALUE(rubrique)%%>', html_attr($row['rubrique']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(rubrique)%%>', urlencode($urow['rubrique']), $templateCode);
		...
		$templateCode = str_replace('<%%VALUE(rubrique)%%>', '', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(rubrique)%%>', urlencode(''), $templateCode);
	...
	$rdata = $jdata = get_defaults('ventilation');
	if($selected_id) {
		$jdata = get_joined_record('ventilation', $selected_id);
		if($jdata === false) $jdata = get_defaults('ventilation');
		$rdata = $row;
	}
	$templateCode .= loadView('ventilation-ajax-cache', array('rdata' => $rdata, 'jdata' => $jdata));

	// hook: ventilation_dv
	if(function_exists('ventilation_dv')) {
		$args=[];
		ventilation_dv(($selected_id ? $selected_id : FALSE), getMemberInfo(), $templateCode, $args);
	}
